...
// ── Image Inspection (Dive-style layer analysis) ─────────────────────────

async function inspectImage(id) {
  const cacheKey = 'docker:image-inspect:' + id;
  const cached = neuralCache.get(cacheKey);
  if (cached) return cached;
  const { data } = await dockerRequest('GET', '/images/' + id + '/json');
  neuralCache.set(cacheKey, data, 120000);
  return data;
}

async function imageHistory(id) {
  const cacheKey = 'docker:image-history:' + id;
  const cached = neuralCache.get(cacheKey);
  if (cached) return cached;
  const { data } = await dockerRequest('GET', '/images/' + id + '/history');
  neuralCache.set(cacheKey, data, 120000);
  return data;
}

async function analyzeImageLayers(id) {
  const [inspection, history] = await Promise.all([
    inspectImage(id),
    imageHistory(id),
  ]);

  const totalSize = inspection.Size || 0;
  const layers = (Array.isArray(history) ? history : []).map((h, i) => {
    let cmd = h.CreatedBy || '';
    cmd = cmd.replace(/^\/bin\/sh\s+-c\s+#\(nop\)\s+/, '').replace(/^\/bin\/sh\s+-c\s+/, 'RUN ');
    if (cmd.length > 200) cmd = cmd.slice(0, 197) + '...';
    return {
      index: i,
      id: (h.Id || '<missing>').replace('sha256:', '').slice(0, 12),
      createdBy: cmd,
      size: h.Size || 0,
      sizeFormatted: formatBytes(h.Size || 0),
      created: h.Created || 0,
      tags: h.Tags || [],
      empty: (h.Size || 0) === 0,
    };
  ...
  const substantiveLayers = layers.filter(l => !l.empty);
  const emptyLayers = layers.length - substantiveLayers.length;
  const largestLayer = substantiveLayers.reduce((max, l) => l.size > max ? l.size : max, 0);

  // Detect optimization targets
  const optimizationTargets = [];
  layers.forEach(l => {
    if (l.size > totalSize * 0.4 && substantiveLayers.length > 1) {
      optimizationTargets.push({ layer: l.index, command: l.createdBy, size: l.size, sizeFormatted: l.sizeFormatted, reason: 'Layer uses >40% of total image size' });
    }
  });

  // Check for duplicate package manager operations
  const pkgOps = layers.filter(l => /apt-get|apk add|yum install|npm install|pip install|gem install/i.test(l.createdBy));
  if (pkgOps.length > 1) {
    pkgOps.forEach(l => {
      optimizationTargets.push({ layer: l.index, command: l.createdBy, reason: 'Multiple package manager operations — consider combining into single RUN' });
    });
  }

  // Efficiency score
  let efficiencyScore = 1.0;
  if (layers.length > 20) efficiencyScore -= 0.05;
  if (pkgOps.length > 2) efficiencyScore -= 0.05;
  if (emptyLayers > layers.length * 0.5) efficiencyScore -= 0.1;
  if (totalSize > 1073741824) efficiencyScore -= 0.05; // >1GB
  efficiencyScore = Math.max(0, Math.min(1, efficiencyScore));

  return {
    imageId: (inspection.Id || '').replace('sha256:', '').slice(0, 12),
    fullId: inspection.Id,
    ...
    dockerVersion: inspection.DockerVersion || '',
    author: inspection.Author || '',
    layerCount: layers.length,
    substantiveLayerCount: substantiveLayers.length,
    emptyLayerCount: emptyLayers,
    largestLayer: formatBytes(largestLayer),
    largestLayerBytes: largestLayer,
    ...
    config: {
      ...
      envCount: (inspection.Config?.Env || []).length,
      exposedPorts: Object.keys(inspection.Config?.ExposedPorts || {}),
      workingDir: inspection.Config?.WorkingDir || '',
      user: inspection.Config?.User || 'root',
      volumes: Object.keys(inspection.Config?.Volumes || {}),
    },
  };
}
...
module.exports = {
  ...
  listImages, pullImage, removeImage, inspectImage, imageHistory, analyzeImageLayers,
