...
function getDefaultPassphrase() {
  return getManagedSecret({
    envNames: ['VAULT_KEY', 'ENCRYPTION_KEY'],
    filePath: VAULT_KEY_FILE,
    legacyValues: [require('os').hostname() + '-bulwark'],
  });
}

function getPassphraseCandidates(passphrase) {
...
function decrypt(enc, passphrase) {
  ...
  let lastError = null;
  for (const candidate of getPassphraseCandidates(passphrase)) {
    try {
  ...
  throw lastError || new Error('Could not decrypt credential');
}
